...
public class ShipMovement : MonoBehaviour
{
    ...
    [SerializeField] private KeyCode toggleStabilizers;
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(toggleStabilizers)){
            ToggleStabilizers();
        }
    ...
    void ToggleStabilizers()
    {
        if (ship.GetComponent<Bodies>() == null)
        {
            rb.drag = 0;
            ship.AddComponent<Bodies>();
        }
        else
        {
            rb.drag = 0.5f;
            Destroy(ship.GetComponent<Bodies>());
        }
    }
}
